...
var findTextOnPage = function findTextOnPage(page, text) {
  ...
  for (var i = 0, j = 0; i < pageWords; i++, j = 0) {
    // Seach for a match between the first part of the order number and the text
...
var REDACT_NAME = 'RAJ SHUKLA';
var MATCH_CASE = false;
var REGEX_FLAGS = '';
var REDACT_ORDER_NUMBER = false;
var DEBUG = false; // Prints to console when true
...
var initailizationDialog = {
  initialize: function initialize(dialog) {
    dialog.load({
      name: REDACT_NAME,
      ckb1: MATCH_CASE,
      ckb2: REDACT_ORDER_NUMBER,
      ckb3: DEBUG
    });
  },
  ckb1: function ckb1() {
  ...
  ckb2: function ckb2() {
    this.hasOrderNumber = !this.hasOrderNumber;
  },
  ckb3: function ckb4() {
    this.debug = !this.debug;
  },
  commit: function commit(dialog) {
    var results = dialog.store();
    REDACT_NAME = results.name;
    MATCH_CASE = results.ckb1;
    REDACT_ORDER_NUMBER = results.ckb2;
    DEBUG = results.ckb3;
  },
...
if (PAGES >= 1 || getPageNumWords(0) > 0) {
  app.execDialog(initailizationDialog);
  if (!EXIT) {
    // INITIALIZATION
    if (DEBUG) {
      console.show();
      console.clear();
      console.println('----- BEG DOC DATA -----');
      console.println('Total Pages: ' + PAGES);
      for (var d = 0; d < PAGES; d++) {
        console.println('Page ' + (d + 1) + ': ' + getPageNumWords(d) + ' words');
      }
      console.println('----- END DOC DATA -----\n');
    }
    ...
    if (REDACT_NAME) {
      ...
      var regExpNameToFind = REDACT_NAME.split(' ');
      regExpNameToFind.forEach(function forEach(word, key) {
        regExpNameToFind[key] = new RegExp(word, REGEX_FLAGS);
      });
      ticketData.newData('name', regExpNameToFind, new RegExp(REDACT_NAME, REGEX_FLAGS));
    }
    if (REDACT_ORDER_NUMBER) {
      ticketData.newData('orderNumber');
    }
    for (var page = 0; page < PAGES; page++) {
      if (DEBUG) {
        console.println('************ BEGIN PAGE ' + (page + 1) + ' ************');
      }
      if (REDACT_NAME) {
        ticketData.addData('name', page);
        ticketData.redactData('name', page);
      }
      if (REDACT_ORDER_NUMBER) {
        ticketData.addData('orderNumber', page);
        ticketData.redactData('orderNumber', page);
      }
      if (DEBUG) {
        console.println('************ ENDED PAGE ' + (page + 1) + ' ************\n');
      }
    }
    var alertCode = 3;
    var numRedacted;
    var nameRedactionString = '';
    if (REDACT_NAME) {
      nameRedactionString = '\nInstances of Name Redactions:\t';
      numRedacted = ticketData.getInstances('name', 'redactions') || 0;
      numRedacted /= REDACT_NAME.split(' ').length;
      nameRedactionString += numRedacted;
      if (numRedacted % PAGES === 0) {
        nameRedactionString += '\t\u2713';
      } else {
        nameRedactionString += '\tX';
        alertCode = 0;
      }
    }

    var orderNumberRedactionString = '';
    if (REDACT_ORDER_NUMBER) {
      orderNumberRedactionString = '\nInstances of Order Number Redactions:\t';
      numRedacted = ticketData.getInstances('orderNumber', 'redactions') || 0;
      orderNumberRedactionString += numRedacted;
      if (numRedacted % PAGES === 0) {
        orderNumberRedactionString += '\t\u2713';
      } else {
        orderNumberRedactionString += '\tX';
        alertCode = 0;
      }
    }
    ...
    app.alert('Completed Redactions.\n' + nameRedactionString + '\n' + orderNumberRedactionString, alertCode);
  }
}
